...
Three properties matter: candidates **overlap** (same primitive can belong to multiple supervectors — the NMM cover-system analogy), candidates carry **provenance** (formation rule, member primitives, source layers, all auditable), and IDs are **stable** (re-running formation on the same input produces the same IDs so the per-node cache can invalidate locally on edits).
...
2. **Specialize with small heads.** Freeze the encoder; attach task-specific heads (linear, small MLP, or BT density-ratio for the kalomaze ranker). Each is a few thousand to a few million parameters and trains in minutes on commodity hardware. Most labels are programmatic.
