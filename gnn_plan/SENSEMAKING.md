...
> build small, calibrated graph tools over deterministically formed
> primitive/supervector candidates, with HATCH-IoU as the first
> self-supervised correctness signal, then use uncertainty and expert feedback
> to discover rules/forms that improve coverage and consistency of the
...
- Prototype memory, embedding probes, and later interpretability tools matter
  only when they help decode structured intent: carrier style, hatch/outline
  equivalence, wall continuity, decomposition patterns, closure failures, snap
  regimes, etc.
...
- pair-relation scoring and supervector classification where sparse geometry
...
- embedding neighborhoods for recurring failure families, after the sparse
  baseline and calibration harness exist
...
3. `TALK_TRACK.md` — what to say live
4. `13_primary_answer.md` — consolidated post-call answer
5. `17_dxf_tokenization.md` — M1 tokenization reference
6. `SYSTEM_SPEC.md` — what to build
7. `SENSEMAKING.md` — why this shape matters
8. `06_example_insights.md` — what the first received examples changed
9. `04_research_map.md` — evidence shelf
